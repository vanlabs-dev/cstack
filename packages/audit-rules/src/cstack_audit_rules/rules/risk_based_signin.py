...
METADATA = RuleMetadata(
    ...
    title="Sign-in risk policy missing",
    severity=Severity.HIGH,
    description=(
        "Tenant should have an enabled CA policy that triggers on "
        "signInRiskLevels of medium or high so risky sessions get challenged "
        "or blocked."
    ),
    references=[
    ...
    remediation_hint=(
        "Add an enabled CA policy with conditions.signInRiskLevels including "
        "medium and high, requiring MFA or blocking access."
    ),
)
...
def _evaluate(context: AuditContext) -> list[Finding]:
    qualifying = [
        ...
        and p.conditions.sign_in_risk_levels
        and bool(set(p.conditions.sign_in_risk_levels) & {"medium", "high"})
    ]
    if qualifying:
        return []
    return [
        make_finding(
            ...
            summary="No enabled CA policy uses signInRiskLevels.",
